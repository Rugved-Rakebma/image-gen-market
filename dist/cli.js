...
function parseBrandProfile(filePath) {
  const absPath = path.resolve(filePath);
  if (!fs.existsSync(absPath)) {
...
function augmentPromptWithBrand(prompt, brand) {
  const parts = [prompt];
  if (brand.style) {
  ...
  if (Object.keys(brand.colors).length > 0) {
    const colorStr = Object.entries(brand.colors).map(([k, v]) => `${k}: ${v}`).join(", ");
...
function resolveOutputPath(userPath, prefix = "generated") {
  if (userPath) {
...
program2.command("generate").description("Generate an image from a text prompt").argument("<prompt>", "Text prompt describing the image to generate").option("-o, --output <path>", "Output file path").option("-a, --aspect <ratio>", "Aspect ratio (1:1, 16:9, 9:16, 4:3, 3:4)").option("-m, --model <model>", "Model to use (flash, pro)", "flash").option("-s, --seed <number>", "Seed for reproducible generation", parseInt).option("--size <size>", "Image size hint").option("-r, --ref <paths...>", "Reference image paths").option("--brand <path>", "Brand profile markdown file path").option("--check-env", "Only check if GEMINI_API_KEY is set").action(async (prompt, options) => {
  if (options.checkEnv) {
    ...
    console.log(JSON.stringify(
      hasKey ? { success: true, message: "GEMINI_API_KEY is set" } : { success: false, error: "GEMINI_API_KEY environment variable is not set. Get a key at https://aistudio.google.com/apikey" }
    ));
    return;
  }
...
program2.command("edit").description("Edit an existing image with instructions").argument("<image>", "Path to the image to edit").argument("<instruction>", "Editing instruction describing the changes").option("-o, --output <path>", "Output file path").option("-a, --aspect <ratio>", "Aspect ratio (1:1, 16:9, 9:16, 4:3, 3:4)").option("-m, --model <model>", "Model to use (flash, pro)", "flash").option("--size <size>", "Image size hint").option("--check-env", "Only check if GEMINI_API_KEY is set").action(async (image, instruction, options) => {
  if (options.checkEnv) {
    const hasKey = !!process.env.GEMINI_API_KEY;
    console.log(JSON.stringify(
      hasKey ? { success: true, message: "GEMINI_API_KEY is set" } : { success: false, error: "GEMINI_API_KEY environment variable is not set. Get a key at https://aistudio.google.com/apikey" }
